// DON'T TOUCH THESE...

window.languageSet = [];
window.languageSet['en'] = [];
window.languageSet['zh'] = [];
window.languageSet['en']['clue'] = [];
window.languageSet['zh']['clue'] = [];

// LANDING PAGE

window.languageSet['en']['title-landing'] = 'Old Changi Hospital ';
window.languageSet['zh']['title-landing'] = 'zh-title-landing';

window.languageSet['en']['intro'] = 'Evil takes form in the derelict hallways of <span style="white-space: nowrap;" >Old Changi Hospital </span> as vengeful souls rise once again in the dead of the night. <br / >Will Singapore\'s bloodiest history repeat itself?';
window.languageSet['zh']['intro'] = 'zh-intro';

window.languageSet['en']['instruction'] = 'TO SURVIVE THIS TEST, YOU MUST KNOW EVIL BY NAME ';
window.languageSet['zh']['instruction'] = 'zh-instruction';

window.languageSet['en']['icon-text-1'] = 'Look for hidden clues with the help of an EMF indicator.';
window.languageSet['zh']['icon-text-1'] = 'zh-icon-text-1';

window.languageSet['en']['icon-text-2'] = 'Snap a photo when the EMF indicator peaks.';
window.languageSet['zh']['icon-text-2'] = 'zh-icon-text-2';

window.languageSet['en']['icon-text-3'] = 'Collect all 3 clues to unravel the name.';
window.languageSet['zh']['icon-text-3'] = 'zh-icon-text-3';

window.languageSet['en']['icon-text-4'] = 'Time is the key to higher chances of winning.';
window.languageSet['zh']['icon-text-4'] = 'zh-icon-text-4';

window.languageSet['en']['start-button'] = 'START NOW';
window.languageSet['zh']['start-button'] = 'zh-start-now';

// FAIL PAGE

window.languageSet['en']['title-fail'] = 'The Past Has<br/>Caught To You';
window.languageSet['zh']['title-fail'] = 'zh-title-fail';

window.languageSet['en']['intro-fail'] = 'However, history tends to repeat itself.<br/>You\'ve been given another chance to unravel<br/>the true name of evil.';
window.languageSet['zh']['intro-fail'] = 'zh-intro-fail';

window.languageSet['en']['retry-button'] = 'TRY AGAIN';
window.languageSet['zh']['retry-button'] = 'zh-retry-button';

// COMPLETE (SHARING) PAGE

window.languageSet['en']['title-share'] = 'Spread The Word Of Evil';
window.languageSet['zh']['title-share'] = 'zh-title-share';

window.languageSet['en']['intro-share'] = 'Stand a chance to win 2 pairs of ticket to Halloween Horror Nights 6 by sharing your survival with friends.';
window.languageSet['zh']['intro-share'] = 'zh-intro-share';

window.languageSet['en']['share-and-win'] = 'SHARE & WIN NOW ';
window.languageSet['zh']['share-and-win'] = 'zh-share-and-win';

window.languageSet['en']['your-time'] = 'YOUR TIME ';
window.languageSet['zh']['your-time'] = 'zh-your-time';

window.languageSet['en']['terms-and-conditions'] = 'Terms & conditions ';
window.languageSet['zh']['terms-and-conditions'] = 'zh-terms-and-conditions';

window.languageSet['en']['terms-and-conditions-apply'] = 'apply.';
window.languageSet['zh']['terms-and-conditions-apply'] = 'zh-terms-and-conditions-apply';

window.languageSet['en']['leaderboard'] = 'LEADERBOARD';
window.languageSet['zh']['leaderboard'] = 'zh-leaderboard';

window.languageSet['en']['replay'] = 'REPLAY';
window.languageSet['zh']['replay'] = 'zh-replay';

window.languageSet['en']['code-instruction'] = 'Collect all 5 codes to<br/> unlock the final haunting.';
window.languageSet['zh']['code-instruction'] = 'zh-code-instruction';

window.languageSet['en']['code-download'] = 'DOWNLOAD IMAGE';
window.languageSet['zh']['code-download'] = 'zh-code-download';

// GAME

window.languageSet['en']['who-am-i'] = 'Who Am I?';
window.languageSet['zh']['who-am-i'] = 'zh-who-am-i';

window.languageSet['en']['not-in-my-name-1'] = 'THERE\'S NO';
window.languageSet['zh']['not-in-my-name-1'] = 'zh-not-in-my-name-1';

window.languageSet['en']['not-in-my-name-2'] = 'IN MY NAME';
window.languageSet['zh']['not-in-my-name-2'] = 'zh-not-in-my-name-2';

window.languageSet['en']['tries-left-1'] = 'YOU HAVE ';
window.languageSet['zh']['tries-left-1'] = 'zh-trials-left-1';

window.languageSet['en']['tries-left-2'] = 'TRIES LEFT ';
window.languageSet['zh']['tries-left-2'] = 'zh-trials-left-2';

window.languageSet['en']['enter-letter'] = 'Enter a letter to unravel the name';
window.languageSet['zh']['enter-letter'] = 'zh-enter-letter';

window.languageSet['en']['enter-button'] = 'ENTER';
window.languageSet['zh']['enter-button'] = 'zh-enter-button';

window.languageSet['en']['not-yet-over'] = 'This Is Yet To be Over';
window.languageSet['zh']['not-yet-over'] = 'zh-not-yet-over';

window.languageSet['en']['how-many-nights'] = 'How many nights will Halloween Horror Nights 6 be held?';
window.languageSet['zh']['how-many-nights'] = 'zh-how-many-nights';

window.languageSet['en']['collected-all-clue-copy'] = 'You have collected all the clues.';
window.languageSet['zh']['collected-all-clue-copy'] = 'zh-collected-all-clue-copy';

window.languageSet['en']['guess-name-button'] = 'GUESS THE NAME NOW';
window.languageSet['zh']['guess-name-button'] = 'zh-guess-name-button';
